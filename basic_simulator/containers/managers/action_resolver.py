...
import managers.action_resolvers.generic
...
from managers.action_resolvers import food as harvest_resovlers
...
class GenericResolverContainer(containers.DeclarativeContainer):
    ...
    resolver = providers.Singleton(managers.action_resolvers.generic.GenericActionResolver,
...
class HarvestResolverContainer(containers.DeclarativeContainer):
    config = providers.Configuration('config')
    logger = DirectNotify().newCategory("harvest_resolver")

    resolver = providers.Singleton(harvest_resovlers.HarvestResolver, logger=logger)
